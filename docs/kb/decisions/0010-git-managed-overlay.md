...
- **Evidence**: `cli/src/config/mod.rs` (`presets_overlay_git`, `managed_overlay_dir`,
  ...
  (`sync_managed_overlay`), `cli/src/presets_commands.rs` (`shine overlay link --git`)

## Context

An overlay could already be a Git checkout, but every machine had to `git clone` it somewhere and
then `shine overlay link <path>` that path. For a user who maintains the overlay on one machine and
...
(so it follows `SHINE_CONFIG_DIR`). `shine pull` clones it `--depth 1` on first use, then
**force-mirrors** it to the remote tip on every subsequent run: `git fetch --depth 1 origin
<branch>` followed by `git reset --hard FETCH_HEAD`. `shine overlay link --git <url>` writes the
...
- The old checkout stays usable when a sync fails: the fetch runs before the reset (an unreachable
  ...
  existing on disk, so a failed `shine pull` leaves the last-good overlay in place.
