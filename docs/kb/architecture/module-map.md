...
| `core/src/runtime/shell_action_executor.rs` | Phase 4 Shell launcher create/update/remove and raw external category snapshot replacement actions, per-platform/file-tree rollback journal, receipt/positive-marker-gated commit, and explicit fingerprint-guarded recovery |
