...
| `core/src/runtime/action_executor.rs` | Phase 4 App managed-file create/update/ordinary-or-forced-remove journal, manifest-receipt-gated commit, privileged/unprivileged path dispatch, persistent backup restoration, same-directory rollback material, explicit recovery Plan, and fingerprint-guarded remove/restore |
