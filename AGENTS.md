...
└── presets/      # Embedded assets (compiled into binary via rust-embed)
    ...
    │   ├── clash-verge/ merge.yaml (inert commented EXAMPLE — real copy lives in the overlay under the same name with values hardcoded, no templating so always valid YAML, like surge's local-*.conf), build.ts + unbuild.ts (bun), shine.toml  (dest = ~/.shine/clash-verge; plain Copy, no transforms; CVR Merge profile = proxies + proxy-groups + rule-providers + prepend-rules. Shares Surge's rules/ source + LAN server. Artifact runtime = "bun" (build.ts/unbuild.ts run via `bun`, cross-platform — CVR is Win/mac/Linux, unlike surge's macOS-only bash artifact). `shine app build clash-verge` (build.ts) (1) writes the resolved merge.yaml — overlay copy wins via SHINE_APP_OVERLAY_DIR — into CVR's Global Extend Config file `<CVR data dir>/profiles/Merge.yaml` (fixed name; path auto-detected per platform via APPDATA/HOME, or CLASH_MERGE_FILE override — a per-machine value, not shared overlay), idempotently, so no manual paste; and (2) refreshes mihomo rule-providers via the external controller (CLASH_CONTROLLER_URL/TOKEN, plaintext, not `_SECRET`). shine writes only that user-content merge file (like surge writes SURGE_PROFILE), never profiles.yaml/binding/cache — so the user still creates an empty Global Extend Config once to register the slot. `post_install`/`post_upgrade` re-invoke `shine app build clash-verge` (so the write+refresh runs automatically after an install/upgrade that changes merge.yaml — requires `shine` on PATH + allow_app_hooks; only fires on merge.yaml changes, not rule edits which go through upload_surge). See docs/clash-verge-local-subscription-prd.md)
