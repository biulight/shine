...
`--replace-managed` 会覆盖安装后被用户修改的受管内容。先使用 `shine info <TARGET> --diff` 检查差异。`app uninstall --force` 会删除被用户修改过的受管文件，执行前应加 `--dry-run` 预览。对于符合条件的静态 Copy，该强制删除会写入 journal，并把修改后的文件作为同目录 rollback material 暂存到 receipt commit；管理员静态 Copy 通过 privileged move 使用同一事务，其他安装策略仍使用原有卸载路径。
...
管理员静态 Copy 的 recovery 仅在精确恢复状态需要 move 或 remove 受保护路径时包含 administrator
permission。Shine 会在 recovery Plan 获批后请求授权；仅修复 receipt 或清理 stale journal 不会请求。
