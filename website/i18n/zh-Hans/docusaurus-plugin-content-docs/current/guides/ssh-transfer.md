...
混合 payload 沿用本机释放批准和完整工作区、源文件快照检查。在 broker 所在本机的全局
配置设置 `hybrid_decrypt_backend`；远端请求不能选择其后端或 identity。只有通过释放
授权后才在本机选择并解封，取消后不回退。远端只收到获批准的值，不接触数据密钥或私钥。
重新封存或改变收件人会改变已审查快照，需按现有策略规则重新审核。
